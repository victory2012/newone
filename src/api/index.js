...
export default {
  ...
  /* 获取电池单体型号列表 */
  batterySingleModel: () => {
    return http.get(`/dic?type=SingleModel&categoryId=2&t=${new Date().getTime()}`);
  },
  ...
  /* 获取未绑定设备 */
  DeviceList: () => {
    return http.get(`/device/code?status=0&bindingStatus=0&t=${new Date().getTime()}`);
  },
  ...
  overviewCompany: () => {
    return http.get(`/battery_group/sub_companies/count?t=${new Date().getTime()}`);
  },
  ...
  realData: (hostId, device, startTime, endTime) => {
    return http.get(`/battery_group/${hostId}/${device}/data?startTime=${startTime}&endTime=${endTime}&t=${Math.random()}`);
  },

  /* 获取历史数据 */
  historyData: (hostId, device, startTime, endTime) => {
    return http.get(`/battery_group/${hostId}/${device}/data2?startTime=${startTime}&endTime=${endTime}&t=${Math.random()}`);
  },

  /* 历史告警 */
  historyAlarm: (hostId, startTime, endTime, data) => {
    return http.get(`/battery_group_event?hostId=${hostId}&startTime=${startTime}000000&endTime=${endTime}235959&t=${Math.random()}`, data);
  },

  /* 历史补水 */
  historyFluid: (hostId, device, data) => {
    return http.get(`/battery_group/${hostId}/${device}/fluid?t=${Math.random()}`, data);
  },

  /* 单个设备 告警数据 */
  singleAlarm: (hostId, pageData) => {
    return http.get(`/battery_group_event?hostId=${hostId}&t=${Math.random()}`, pageData);
  },
  ...
  noticeUsersList: (data) => {
    return http.get(`/company_global_internal_notice/users?t=${Math.random()}`, data);
  },
  ...
  /* 查询电池组型号 阈值 */
  getBatteryPolicy: (id) => {
    return http.get(`/battery_group_event_policy?modelId=${id}&t=${Math.random()}`);
  },
  ...
  companyPair: (companyId) => {
    return http.get(`/battery_group/${companyId}/id_name_pair?t=${Math.random()}`);
  },
